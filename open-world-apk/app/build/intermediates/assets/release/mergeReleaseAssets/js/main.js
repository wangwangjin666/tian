...
class Game {
  constructor() {
    ...
    this.harvest = new HarvestSystem(this.world, this.inventory);
    this.renderer.harvestSystem = this.harvest;
  ...
  _setupUI() {
    ...
    if (interactBtn) {
      interactBtn.addEventListener('click', () => this._tryHarvest());
    }
  }

  /** 尝试采集当前交互目标 */
  _tryHarvest() {
    if (this.paused || !this._interactTarget) return;
    const result = this.harvest.harvest(this._interactTarget);
    if (result) {
      this._interactTarget = null;
      this._showToast(`+${result.count} ${RESOURCE_NAMES[result.resource]}`);
    }
  }
  ...
  _update(dt) {
    ...
    // 资源重生检查
    this.harvest.updateRespawns();

    // 查找附近可交互目标
    this._interactTarget = this.harvest.findNearest(this.player.x, this.player.y);
  ...
  _render(dt) {
    this.renderer.render(this.player, this.npcs, dt, this._interactTarget);
    this.minimap.render();
  }
  ...
  _updateHUD() {
    ...
    // 背包
    const inv = this.inventory.items;
    document.getElementById('hud-inventory').textContent =
      `木 ${inv.wood} · 石 ${inv.stone} · 花 ${inv.flower} · 果 ${inv.berry}`;

    // 交互按钮：有目标时显示并标注资源名
    const interactBtn = document.getElementById('btn-interact');
    if (interactBtn) {
      if (this._interactTarget && !this.paused) {
        const rule = HARVEST_RULES[this._interactTarget.deco.type];
        interactBtn.style.display = 'flex';
        interactBtn.textContent = `采集 ${RESOURCE_NAMES[rule.resource]}`;
      } else {
